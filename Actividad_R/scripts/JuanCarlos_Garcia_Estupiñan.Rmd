...
## lo que nos hace obterner un vector en el que cada valor son estas \n. En mi caso que 
...
## Básicamente crearemos un verctor desde la posición 1, primer header hasta el último, de 2 en 2
header <- ncrna_vector_preproc[seq(1,longitud_ncrna_vector,2)]
length(header)
## Lo mismo para las secuencias, que empiezan en el valor 2 del vector y van 2 a 2
secuencia <- ncrna_vector_preproc[seq(2,longitud_ncrna_vector,2)]

## Deberían ser la misma longtud ambos en ese sentido
length(header) == length(secuencia)

## Y creamos un data frame con header y secuencia
df_ncrna <- tibble(
    header=header,
    secuencia=secuencia
  )

# Número de secuencias que empiezan con acct
n_secuencias_acct <- length(grep(glue("^{empieza_acct}"),df_ncrna$secuencia))
n_secuencias_acct

```

Además esta sería otra forma de hacerlo de forma más sencilla en caso de encontrarnos en un ordenador que peresente un sitema UNIX con la shell de BASH. Para ello usamos la función ```system()```, que permite correr la CLI en un script de R.
